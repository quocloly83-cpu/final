...
const SESSION_SECRET =
  process.env.SESSION_SECRET || "ath_super_secret_huy_fanta_2026_final";
...
const LINKS = {
  ...
  ff: "https://ff.garena.com/vn/",
  ffmax: "https://ff.garena.com/vn/"
};
...
function renderLogo(size = 76) {
  if (hasLogo()) {
    return `<img src="/logo.png" alt="logo" style="width:${size}px;height:${size}px;object-fit:cover;display:block;border-radius:${Math.round(size * 0.28)}px">`;
  }
  return `<div style="width:${size}px;height:${size}px;border-radius:${Math.round(size * 0.28)}px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#8c52ff,#ff70c7);font-size:${Math.round(size * 0.38)}px;">⚡</div>`;
}
...
function pageStyles() {
  ...
  <style>
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
    :root{
      --bg:#06040b;--card:rgba(12,10,20,.92);--line:rgba(255,255,255,.08);
      --violet:#d8b4ff;--violet2:#b77cff;--pink:#ff6fd8;--muted:#b9b1c9;
      --ok:#8dffb4;--err:#ff7aa2;--gold:#ffd56b;
    }
    body{
    ...
    @keyframes glow{0%{box-shadow:0 0 18px rgba(183,124,255,.16),0 0 36px rgba(255,111,216,.05)}50%{box-shadow:0 0 28px rgba(183,124,255,.25),0 0 56px rgba(255,111,216,.10)}100%{box-shadow:0 0 18px rgba(183,124,255,.16),0 0 36px rgba(255,111,216,.05)}}
    @keyframes pulseText{0%{text-shadow:0 0 10px rgba(183,124,255,.25)}50%{text-shadow:0 0 18px rgba(255,111,216,.25)}100%{text-shadow:0 0 10px rgba(183,124,255,.25)}}
    ...
    .card{width:min(95vw,580px);border-radius:30px;background:var(--card);border:1px solid rgba(215,180,255,.18);animation:glow 4s infinite;overflow:hidden;backdrop-filter:blur(16px)}
    .top{padding:22px 18px 16px;border-bottom:1px solid var(--line);position:relative;overflow:hidden}
    .top::before{content:"";position:absolute;inset:auto -10% -60% auto;width:260px;height:260px;background:radial-gradient(circle, rgba(183,124,255,.16), transparent 65%);pointer-events:none}
    .top::after{content:"";position:absolute;left:-20%;top:-40px;width:140%;height:4px;background:linear-gradient(90deg,transparent,var(--violet2),var(--pink),transparent);background-size:200% 100%;animation:neonBar 3s linear infinite}
    .brand{display:flex;align-items:center;gap:14px}
    .logoBox{width:76px;height:76px;border-radius:22px;overflow:hidden;box-shadow:0 0 18px rgba(183,124,255,.35);flex:0 0 76px;background:rgba(255,255,255,.04)}
    ...
    .btn,.smallBtn,.tab{border:none;color:#fff;cursor:pointer;font-weight:700;border-radius:14px}
    .btn{
      width:100%;height:54px;margin-top:12px;background:linear-gradient(90deg,#8c52ff,#c86bff,#ff70c7);
    ...
    .timeBox{
      margin-top:12px;padding:14px;border-radius:18px;background:linear-gradient(180deg, rgba(255,255,255,.06), rgba(255,255,255,.03));
      border:1px solid var(--line);position:relative;overflow:hidden
    }
    .timeBox::before{content:"";position:absolute;inset:auto -30px -50px auto;width:180px;height:180px;background:radial-gradient(circle, rgba(255,255,255,.08), transparent 65%)}
    .expire{font-size:13px;color:#f2eaff;position:relative;z-index:1}
    .countdown{margin-top:8px;font-size:26px;font-weight:900;color:var(--violet);letter-spacing:1px;position:relative;z-index:1}
    ...
    .tile::before{content:"";position:absolute;width:140px;height:140px;right:-40px;bottom:-40px;background:radial-gradient(circle, rgba(183,124,255,.16), transparent 65%)}
    ...
    .switch input:checked + .slider{background:linear-gradient(90deg,#8c52ff,#ff70c7);box-shadow:0 0 18px rgba(200,107,255,.25)}
    ...
    .rangeLabel{display:flex;align-items:center;justify-content:space-between;font-size:12px;color:#e5dcf5;margin-bottom:8px}
    input[type=range]{width:100%;accent-color:#c86bff}
    .toast{
      position:fixed;left:50%;bottom:18px;transform:translateX(-50%) translateY(20px);min-width:220px;max-width:92vw;padding:14px 16px;
      border-radius:16px;background:rgba(12,15,24,.95);border:1px solid var(--line);color:#fff;text-align:center;z-index:120;opacity:0;pointer-events:none;transition:.25s
    }
    ...
    .splash{
      ...
      background:radial-gradient(circle at center, rgba(170,90,255,.18), transparent 30%), linear-gradient(160deg,#030207,#0b0612,#05040b);
      transition:opacity .6s ease, visibility .6s ease;
    }
    .splash.hide{opacity:0;visibility:hidden}
    .splashLogo{width:170px;height:170px;border-radius:28px;overflow:hidden;box-shadow:0 0 30px rgba(183,124,255,.28),0 0 70px rgba(255,111,216,.12);animation:glow 3s infinite, popIn .7s ease;background:rgba(255,255,255,.03)}
    .splashText{margin-top:18px;font-size:16px;color:var(--violet);font-weight:800;letter-spacing:1px;animation:pulseText 2s infinite}
    .splashSub{margin-top:8px;color:#cbbddf;font-size:12px}
    .adminList{margin-top:14px}
    .item{
      margin-top:12px;padding:14px;border-radius:14px;background:rgba(255,255,255,.05);
      border:1px solid rgba(255,255,255,.08)
    }
    .itemKey{font-weight:700;font-size:16px}
    .itemMeta{margin-top:6px;color:#ddd;font-size:13px;line-height:1.6}
    @media (max-width:560px){
      .tabs{grid-template-columns:repeat(2,1fr)} .grid2{grid-template-columns:1fr}
    }
  </style>
  `;
}
...
function renderHomeHtml() {
  return `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AimTrickHead</title>${pageStyles()}</head>
  <body><div class="wrap"><div class="card"><div class="top"><div class="brand"><div class="logoBox">${renderLogo(76)}</div><div><h1 class="title">AimTrickHead VIP</h1><div class="sub">Hệ thống key đang hoạt động</div><div class="credit">CRE HUY FANTA</div></div></div></div>
  <div class="content"><a class="socialBtn" href="/panel">Vào panel</a><div style="height:10px"></div><a class="socialBtn" href="/admin">Trang admin</a></div></div></div></body></html>`;
}

function renderPanelHtml() {
  return `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AimTrickHead VIP</title>${pageStyles()}</head>
  <body>
  ...
  <div class="wrap">
    <div class="card">
      <div class="top">
        <div class="brand">
          <div class="logoBox">${renderLogo(76)}</div>
          <div>
      ...
      <div class="content">
        <div id="loginView">
          ...
          <div class="grid2">
            <a class="socialBtn" href="${LINKS.zalo}" target="_blank" rel="noopener noreferrer">💬 Zalo</a>
            <a class="socialBtn" href="${LINKS.facebook}" target="_blank" rel="noopener noreferrer">📘 Facebook</a>
          </div>
        ...
        <div id="panelView" class="hidden">
          ...
          <div id="tab3" class="tabPane">
            <div class="grid2">
              <a class="socialBtn" href="${LINKS.zalo}" target="_blank" rel="noopener noreferrer">💬 Liên hệ Zalo</a>
              <a class="socialBtn" href="${LINKS.facebook}" target="_blank" rel="noopener noreferrer">📘 Facebook</a>
            </div>
          ...
          <div id="tab4" class="tabPane">
            <div class="grid2">
              <a class="socialBtn" href="${LINKS.ff}" target="_blank" rel="noopener noreferrer">🎮 Free Fire</a>
              <a class="socialBtn" href="${LINKS.ffmax}" target="_blank" rel="noopener noreferrer">🔥 FF MAX</a>
            </div>
            <div class="footer">Nút mở trang chính thức.</div>
          </div>

          <div id="tab5" class="tabPane">
            <div class="grid2">
              <a class="socialBtn" href="${LINKS.tiktok}" target="_blank" rel="noopener noreferrer">🎵 TikTok</a>
              <a class="socialBtn" href="${LINKS.zalo}" target="_blank" rel="noopener noreferrer">💬 Liên hệ Admin</a>
            </div>
            <div class="footer">
  ...
  <script>
    ...
    async function kiemTraNen() {
      ...
      try {
        const res = await fetch("/api/status", {
          ...
          body: JSON.stringify({ token, device: getDevice() })
        });
    ...
    function dangXuat(expired) {
      ...
      if (expired) setMsg("Key đã hết hạn hoặc bị khóa. Vui lòng đăng nhập lại.", "err");
      else setMsg("", "");
    }
...
function renderAdminHtml() {
  return `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin Key Manager</title>${pageStyles()}</head>
  <body>
  <div class="wrap">
    <div class="card">
      <div class="top">
        <div class="brand">
          <div class="logoBox">${renderLogo(76)}</div>
          <div>
            <h1 class="title">Admin Key Manager</h1>
            <div class="sub">Tạo, xem và xóa key truy cập</div>
            <div class="credit">CRE HUY FANTA</div>
          </div>
        </div>
      </div>
      <div class="content">
        <input id="adminKey" class="input" type="password" placeholder="Nhập admin key">
        <input id="customKey" class="input" style="margin-top:10px" placeholder="Key tùy chọn (để trống = tự tạo)">
        <div class="grid2">
          <input id="uses" class="input" type="number" value="50" placeholder="Số lượt dùng">
          <input id="days" class="input" type="number" value="30" placeholder="Số ngày sử dụng">
        </div>
        <div class="grid2">
          <button class="btn" onclick="taoKey()">Tạo key</button>
          <button class="btn" onclick="taiDanhSach()">Tải danh sách key</button>
        </div>
  ...
  <script>
    ...
    async function taoKey() {
      ...
      if (!adminKey) {
        setResult("Vui lòng nhập admin key.", "err");
        return;
      }

      setResult("Đang tạo key...", "");
      ...
      try {
        ...
        if (!data.ok) {
          setResult(data.error || "Tạo key thất bại.", "err");
          return;
        }

        setResult("Tạo thành công: " + data.key + " | Hết hạn: " + data.expireText, "ok");
        document.getElementById("customKey").value = "";
        taiDanhSach();
      } catch {
        setResult("Lỗi kết nối máy chủ.", "err");
      }
    }
    ...
    async function taiDanhSach() {
      ...
      if (!adminKey) {
        setResult("Vui lòng nhập admin key trước.", "err");
        return;
      }
      ...
      try {
        ...
        if (!data.ok) {
          box.innerHTML = '<div class="err">' + (data.error || "Không tải được danh sách.") + '</div>';
          return;
        }

        if (!data.items.length) {
          box.innerHTML = '<div class="footer">Chưa có key nào.</div>';
          return;
        }
        ...
        for (const item of data.items) {
          html +=
            '<div class="item">' +
              ...
              '<div class="itemMeta">' +
                'Lượt còn: ' + item.uses + '<br>' +
                'Hết hạn: ' + item.expireText + '<br>' +
                'Thiết bị: ' + (item.device || "Chưa khóa") +
              '</div>' +
              '<button class="smallBtn" style="margin-top:10px;width:100%" onclick="xoaKey(\\'' + item.key + '\\')">Xóa key</button>' +
            '</div>';
        }

        box.innerHTML = html;
      } catch {
        box.innerHTML = '<div class="err">Lỗi kết nối máy chủ.</div>';
      }
    }
    ...
    async function xoaKey(key) {
      ...
      try {
        const res = await fetch("/api/delete?admin=" + encodeURIComponent(adminKey), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!data.ok) {
          setResult(data.error || "Xóa key thất bại.", "err");
          return;
        }

        setResult("Đã xóa key: " + key, "ok");
        taiDanhSach();
      } catch {
        setResult("Lỗi kết nối máy chủ.", "err");
      }
